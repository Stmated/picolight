...
void *pattern_fade_between_data(uint16_t len, float intensity)
{
    ...
    instance->time_per_color = 20000 - randint_probability(0, 18000, 1 - intensity);
    instance->hsi_saturation = 0.5 + 0.5 * (randint_probability(0, 10000, 1 - intensity) / (float)10000);
    instance->hsi_intensity = 0.1 + 0.5 * (randint_probability(0, 10000, 1 - intensity) / (float)10000);
    return instance;
}
...
{
    pattern_register(pattern_fade_between, pattern_fade_between_data, pattern_destroyer_default);
}